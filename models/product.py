import json
class Product:
    def __init__(self, id, name, description, price, stock, id_categoria):
...
class Products:
    ...
    def detalhes(cls, id):
        x = cls.listar_id(id)
        if x != None:
            return f"Id: {x.id} - Description: {x.description} - Price: R${x.price:.2f} - Stock: {x.stock}"
    @classmethod
    def salvar(cls):
        ...
        with open("products.json", mode="w") as arquivo:
    ...
    def abrir(cls):
        ...
        cls.objetos = []
        try:
            with open("products.json", mode="r") as arquivo:
                ...
                for obj in objetos_json:
                    ...
                    c = Product(obj["id"], obj["name"], obj["description"], obj["price"], obj["stock"], obj["id_categoria"])
                    # insere o objeto na lista
