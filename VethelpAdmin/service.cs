...
public class Service
{
    ...
    public Service(int id, int recepcionist_id, int vet_id, int animal_id, int procedure_id, int product_id, string description, string date, int status)
    {
        Id = id;
...
{
    // Lista de objetos do tipo Service
    private static List<Service> objetos = new List<Service>();
    private static string _filePath = "services.json"; // Nome do arquivo
    ...
    public static void Inserir(Service obj)
    {
    ...
    public static Service ListarId(int id)
    {
        Abrir(); // Carrega a lista de objetos
    ...
    public static void Atualizar(Service obj)
    {
    ...
    public static void Excluir(Service obj)
    {
        var usuario = ListarId(obj.Id); // Busca o usuário pelo ID
        if (usuario != null)
        {
    ...
    private static void Abrir()
    {
        ...
        if (File.Exists(vethelpPath)) // Verifica se o arquivo existe no caminho correto
        {
            ...
            objetos = JsonConvert.DeserializeObject<List<Service>>(json, _jsonSettings); // Usa as configurações na desserialização
